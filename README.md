...
- **Weather per stop** — hourly forecast from [Open-Meteo](https://open-meteo.com) (no key, no account). Each stop shows the conditions for its own hour; the strip above the banners gives the day's range and worst rain chance. The response is cached in `localStorage`, refreshed at most every 45 minutes, and the last fetch keeps showing when you're offline (labelled with its age once it's over 90 min old).
- **Subway / walking leg** — every stop carries a `transit` line describing how you get there from the previous one (train + direction + where to get off, or the walk). Static by design: it works offline and in the Penn Station dead zones. It is not live service status — check the MTA app if something looks wrong.
